...
def organize_folder(target_directory: Path):
    """
    Organizes files in the target directory into subdirectories based on their extension.

    Args:
        target_directory (Path): The path to the folder to be organized.
    """
    ...
    for item in target_directory.iterdir():
        ...
        if item.is_dir() or item.name == "smart_organizer.py":
            continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="A smart file organizer that sorts files into categorized folders based on their extension.",
        epilog="Example: python smart_organizer.py /path/to/your/Downloads"
    )
    ...
    parser.add_argument(
        "target_directory",
        type=str,
        help="The full path to the directory you want to organize."
    )

    args = parser.parse_args()
    
    # Convert the string path to a Path object for better handling
    directory_to_organize = Path(args.target_directory)
